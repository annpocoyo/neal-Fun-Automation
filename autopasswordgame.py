...
import xml.etree.ElementTree as ET # For analyzing the chess board from an svg
...
def main():
    ...
    autoPasswordGame.passwordContents += "0!XXXV113june00:00pepsi🌑🌒🌓🌔🌕🌖🌗🌘🏋️‍♂️🏋️‍♂️🏋️‍♂️iamloved"
    autoPasswordGame.updateBox()
    
    # Find CAPTCHA
    captcha_code_source = autoPasswordGame.driver.find_elements(By.CLASS_NAME, "captcha-img")[0].get_attribute("src")
    ...
    wordle = requests.get(f"https://www.nytimes.com/svc/wordle/v2/{date:%Y-%m-%d}.json").json()['solution']
    ...
    mapsFile = open(f"{os.path.dirname(os.path.abspath(sys.argv[0]))}/Library/passwordGame/maps.json")
    mapsMatchs = json.load(mapsFile)

    # Find embed url for geoguesser
    geoGuesserEmbed = autoPasswordGame.driver.find_elements(By.XPATH, "//iframe[@class=\"geo\"]")[0].get_attribute("src")
    ...
    # Solve chess
    # Find chess svg URL
    chessSVGURL = autoPasswordGame.driver.find_elements(By.CLASS_NAME, "chess-img")[0].get_attribute("src")
    ...
    chessTurn = autoPasswordGame.driver.find_elements(By.CLASS_NAME, "move")[0].text == "White to move"
    ...
    youtubeRuleWords = autoPasswordGame.driver.find_elements(By.CLASS_NAME, "youtube")[0].find_elements(By.XPATH, ".//*[contains(text(), 'YouTube video')]")[0].text.split(" ")
    ...
    if "second" in " ".join(youtubeRuleWords):
        ...
        youtubeSeconds = int(youtubeRuleWords[youtubeRuleWords.index("second") - 1])
    else:
        # No, so 0 seconds
        youtubeSeconds = 0

    # Load JSON file of YouTube videos
    youtubeVideosFile = open(f"{os.path.dirname(os.path.abspath(sys.argv[0]))}/Library/passwordGame/youtubeVideos.json")
    youtubeVideoMatchs = json.load(youtubeVideosFile)

    # Get youtube video url
    youtubeVideo = "youtu.be/" + youtubeVideoMatchs[str(youtubeMinutes)][youtubeSeconds]
    ...
    # Get color display
    colorDisplay = autoPasswordGame.driver.find_elements(By.CLASS_NAME, "rand-color")[0]

    # Get refresh button
    colorRefresh = colorDisplay.find_elements(By.CLASS_NAME, "refresh")[0]
    ...
    while any(char.isdigit() for char in hexColor):
        ...
        RGBColor: tuple = tuple(map(int, colorDisplay.value_of_css_property("background").replace("rgb(", "").replace(")", "").replace(",", "").split(" ")))
    ...
    # Get password divs
    passwordDiv = autoPasswordGame.driver.find_elements(By.CLASS_NAME, "password-wrapper")[0]
    ...
    while len(textElement) == 0:
        ...
        textElement: list = passwordDiv.find_elements(By.XPATH, ".//*[contains(text(), 'Please re-type your password')]")
        continue # Wait
